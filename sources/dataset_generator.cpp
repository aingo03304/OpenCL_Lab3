...
static void compute(float *output, float *input0, float *input1, int num) {
  for (int ii = 0; ii < num; ++ii) {
    ...
    for (int jj = 0; jj < FILTER_SIZE; ++jj) {
        if (ii-FILTER_RADIUS+jj >= 0 && ii-FILTER_RADIUS+jj < num) {
           ovalue += input0[ii-FILTER_RADIUS+jj] * input1[jj];
        }
    }
    output[ii] = ovalue;
  }
...
static float *generate_filter() {
  float *filter = (float *) malloc(sizeof(float) * FILTER_SIZE);
  for (int i=0;i < FILTER_SIZE; i++) {
    filter[i] = ((float) (rand() % 1000)) / 1000.0f;
  } 
  return filter;
}
...
static void create_dataset(int datasetNum, int dim) {
  ...
  float *input1_data = generate_filter();
  ...
  write_data(input1_file_name, input1_data, FILTER_SIZE);
...
int main() {
  base_dir =
      gpuTKPath_join(gpuTKDirectory_current(), "1DConvolution", "Dataset");
